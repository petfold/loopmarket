...
| Role heads over a base dimension | **fired 2026-09-12** (`P1-spacetime-terms.md` §5.3b): with `from` declared under `geo`, `from(ljubljana)` is accepted and silently read as a literal *cell* named "ljubljana" — the footgun the CLI's value substitution guards against; a region node cannot be a service parameter at all | **To ask upstream**: a head declared under another head accepts that head's *nodes* as parameters and denotes their value (a region's covering), so `from(ljubljana)` stands as a stored term and `from(my_home) ⊑ from(ljubljana)` computes. Meanwhile: role terms carry cell values; the CLI resolves a name to its public cell before publishing. |
| Boolean overlap face          | **fired 2026-09-12** (`P1-spacetime-terms.md` §5.3c): `satisfies` computes overlap per head with `dimensions.intersect` on two terms, which cannot take a region node on either side, and reaches no graph-declared units | **To ask upstream**: `overlaps(a, b)`, the mirror of `is_below` for `get_overlapping`, region nodes included on both sides, units from the store. Meanwhile: `intersect` on cell/time values (complete for `time`/`geo`, which declare no units); region∩region would need enumeration loopmarket does not build. |
