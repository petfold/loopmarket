...
\begin{tikzpicture}
  ...
  \node[note, text width=110mm] at (0,-40mm) {The courier's give and want name the \emph{same} thing at two places and times: delivery is an offer that moves a thing through space and time. Four legs; each maker's legs cancel on its own scale; settlement commits all four or none. A two- or three-leg cycle is only the smallest case.};
\end{tikzpicture}
\end{document}
