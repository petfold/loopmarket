...
- `src/loopmarket/registry.py` — the book: `offer/`, `sig/`, `fill/`, `loop/`, `idx/{c,t,g}/` keyspace over a duck-typed RecordStore; `snapshot()`, `or_set_resolver` for multi-writer merges, `verify_loop_atomicity` (U11), `swarm_offer_book()`.
