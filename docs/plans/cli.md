...
| solver | `loops` (find on a pinned snapshot, print, never clear), `fold` (fold the announced books and peers myself, print the root any manifest must match — built 2026-09-14), `propose` (built 2026-09-15: clear locally and post each loop as a beat on the `beat` contract, `chain:RPC@CONTRACT`; `finalize BEAT` records its fills after the window), `beats [--open]` and `challenge BEAT [LEG] [--check] [--book SPEC]` (built 2026-09-18: the beat's loop record found in the submitter's announced clearing book by hashing to the commitments, every leg re-derived off chain and by the contract's verifier for free, the challenge sent only where the contract would convict; exit 2 when no record exists anywhere); `commit`, `reveal [BEAT]`, `outcome [BEAT] [--check]`, `sealed [BEAT]` (built 2026-09-18 evening: the sealed-proposal beat on the `auction` contract — solve on the fold and seal, open in the reveal phase, derive a closed beat's winners with the reserve bid and the fairness filter and post them) |
...
| plumbing | `set`, `export`, `import`, `help`, `--version`; settings `bond` (my deposit: `QTY[UNIT] CATEGORY... VALUE`, or a bare amount on my scale deposited as `default_asset` at my price), `escrow` (`chain:RPC@CONTRACT`, the `LoopEscrow`; `deposit [ID] [--check]` funds my gives' bonds there in the gas token, `finalize BEAT` then reserves each bonded give's share per fill with the clearing key, `escrow_claim` the claim period after a leg's window — built 2026-09-19), `default_asset` (the configured chain's gas token — `xdai xDAI 1` while Swarm settles on Gnosis — a CLI default the record spells out; an asset category the catalogue lacks is refused at publish), `require_point`, `require_cancel`, `ladder`, `require_accepts`, `require_escrows` (admissibility by declaration, the v5 record accepted 2026-09-19; the render shows the deposit and the requirement) |
...
4. ~~**The whole space of a role**~~ — filed and **resolved the other
   ...
   passes — then, the same night, the overlap relation itself withdrawn:
   every term matches by containment; `from(geo)` stays refused.
