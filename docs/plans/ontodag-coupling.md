...
- **`GeoDisc.intersects` stays the exact truth.** Upstream's determinism
  ...
  **Superseded 2026-09-12** (Peter's ruling, `P1-spacetime-terms.md` §4):
  cells and region nodes *become* the exact truth and the disc leaves the
  record at v3; the planar-frame upgrade is then input convenience, not a
  correctness path. And the relation is the *head's*, not the dimension's:
  heads under the marker `service-role` match by overlap, descriptive
  spacetime terms (`made_in(...)`) by containment (`P1-spacetime-terms.md`
  §3; landed in `Ontology.satisfies` the same day).
...
- **Wiring the indexed generator** (decided 2026-08, lands with P1):
  ...
  **Landed 2026-09-12** (ontodag #14 on main): `candidates` is one `get`
  ...
  `items_only=True`. **Then the question itself was withdrawn (Peter,
  the same night): *why is overlap relevant at all? Ontodag is based on
  intersection.*** A want is the wider cone and a give the narrower one,
  so a want's place and time are query terms like its categories, and
  `candidates` is `get([line marker, *want.concepts], items_only=True)` —
  no overlap terms, no whole-space device for silence (a give that says
  nothing about where it hands over is in no `where` cone, and that is
...
- **One intersection engine** (Peter, 2026-09-07). ontodag is the master
  ...
  after which `candidates` is exactly one `get` — **done 2026-09-12**; the
  marker category per record line does the rest of the "is an offer"
  filtering inside the plan. (ii) The registry's
  ...
  **Decision: retire them** when §2's spacetime terms enter the shared
  ...
  of the same facts (`P1-federated-book.md` §1, §2). **Retired
  2026-09-12** (`index_offers`, `ids_by_index`, the bucket/cell chains
  deleted, and `Manifest.index_root` with them — a root for nothing does
  not stay).
...
| Overlap terms in the query planner (withdrawn — see "Overlap itself" below) | **fired 2026-09-07** (Peter): `DimensionIndex.candidates` runs `get(concepts)` and `get_overlapping(window)` to completion and intersects in Python, so the planner's smallest-first / early-stop logic never crosses dimensions — a category offered only in a few windows or regions can't cut the search short | **Landed 2026-09-12** — [ontodag #14](https://github.com/petfold/ontodag/issues/14): `get(terms, overlapping=[...], items_only=False)`, one plan of three cone kinds, overlap terms never pre-intersected as meets, `items_only` = no parametric value and nothing filed under it. Consumed the same night: `candidates` is one `get`, no set arithmetic on the answer; the `idx/{c,t,g}` index retired (§5). |
| Overlap itself | **fired 2026-09-12 night** (Peter): *why is overlap relevant at all? Ontodag is based on intersection.* The whole overlap story — three queries intersected in Python, #14's `overlapping=` planner argument, a whole-space value for silence, #17's per-candidate probe — read a give's place and time as flexible ranges; a want is the wider cone and a give the narrower | **Withdrawn 2026-09-12 night**, both repos: every term matches by containment, ontodag's `get(overlapping=)` removed (CLI/REST/MCP too), loopmarket's `service-role` marker and overlap half gone, `candidates` = `get(want.concepts)`. `from(geo)` stays refused ([ontodag #17](https://github.com/petfold/ontodag/issues/17), closed that way); #18's cache stays. |
...
| Boolean overlap face (no longer used by loopmarket; `overlaps`/`meet` remain ontodag arithmetic) | **fired 2026-09-12** (`P1-spacetime-terms.md` §5.3c): `satisfies` computes overlap per head with `dimensions.intersect` on two terms, which cannot take a region node on either side, and reaches no graph-declared units | **Landed 2026-09-12** — [ontodag #16](https://github.com/petfold/ontodag/issues/16): `OntoDAG.overlaps(a, b)` and `OntoDAG.meet(a, b)`, nodes either side, units from the store; `meet` raises when no single term names the intersection (a place and a cell it is not known to lie in). Consumed: `satisfies`' overlap half is one `overlaps` per head, `Ontology.meet` folds with `OntoDAG.meet` and fails closed on the raise; `dimensions.intersect`/`canonicalize` left loopmarket. |
| Operator terms: a role over the category graph | **fired 2026-09-13 evening** (Peter: *the parameter of the transport is a want from the transporter's side*): `transport(bicycle)` must order against `transport(small-item)` by the graph, the argument may be a term (`transport(weight(..8kg))`) or a sorted conjunction (`transport(small-item weight(..8kg))` ≡ `transport(small-item) transport(weight(..8kg))`); ontodag's `split_term` is flat and a role's parameter may only name a node of a dimension with a kind | **Resolved the same evening: ontodag 0.26.0, the category kind** (`category-dimension`, registry 4.2, DIMENSIONS.md §15): a head under it takes a conjunction of constraints on the graph, canonical sorted/deduplicated, a redundant constraint refused (I1), ordered by the graph; roles keep their own star. `declare_operator` declares the category under the kind; the facade's own splitter was deleted; `schema.canonical_term` keeps the same sort for the stored spelling (the schema has no catalogue). The direction (want-within-give) stays loopmarket's, declared by the `operator` marker. Not in ontodag's prelude, so no pack root moved. |
