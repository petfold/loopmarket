...
\begin{frame}{Who pays for what}
  ...
  \begin{tabular}{@{}p{24mm}p{58mm}p{48mm}@{}}
    ...
    solver & settlement gas for the loops it wins & an endogenous spread, bounded by the reserve bid \\
    aggregator & its own storage and bandwidth & fees for \emph{serving} fast, never for inclusion \\
    settlement & nothing of its own & nothing: gas is the winner's \\
...
\begin{frame}{Why not reputation-based}
  \begin{itemize}
    ...
    \item loopmarket's rule U12: standing counts settled, cost-borne loops only; loss experience comes from bonded, adjudicated events (factbond).
  \end{itemize}
  \vspace{4mm}
