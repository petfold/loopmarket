...
# `geo`/`time` marked as handover coordinates (the v3 record's spacetime — the
...
CATALOGUE = {
    **{name: list(parents) for name, parents in PRELUDE_DECLARATIONS},
    "handover": [], "geo": ["prefix-dimension", "handover"],
    "time": ["calendar-dimension", "handover"],
...
def _api_triangle(nonces):
    ...
    flat = f"geo({cell_for_coords(46.05, 14.50, 5_000)})"
    farm = f"geo({cell_for_coords(46.10, 14.55, 15_000)})"
    shop = f"geo({cell_for_coords(46.06, 14.51, 4_000)})"
...
def test_g2_two_conventions_and_nothing_else():
    ...
    p = cli.parse_offer_tokens(["ride", "home", "time(today..+7d)", "5"])
    assert p.concepts == ("ride", "home", "time(today..+7d)")   # terms, all
    assert p.heads == {}
...
def test_g4_approval_block_is_the_show_renderer(loop):
    out = loop.ok("give", "10kg", "apple", "home", "100")
    ...
    assert "note     home → geo(" in out       # a private place: its cell
...
def test_unknown_category_fails_closed(loop):
    code, out, err = loop("give", "durian", "home", "5")
...
def test_omitted_price_is_my_last_unit_price_scaled_and_marked(loop):
    loop.ok("give", "10kg", "apple", "home", "100")
...
def test_price_memory_never_reads_other_makers(loop, monkeypatch):
    ...
    code, out, err = loop("give", "5kg", "apple", "home")
...
def test_reused_price_refuses_in_a_batch_under_confirm_auto(loop, monkeypatch):
    ...
    assert loop("give", "5kg", "apple", "home", "40")[0] == 0
...
def test_places_are_catalogue_nodes_with_coordinates(env, tmp_path):
    ...
    fresh = Runner()
    out = fresh.ok("give", "apple", "home", "5")
    ...
    assert f"give     apple geo({cell})" in out
    assert f"note     home → geo({cell})" in out
...
def _od_with_prelude(path, extra_puts):
    ...
    Ontology(dag).declare_handover(
        [h for h in ("geo", "time", "from", "to", "depart", "arrive") if h in dag.nodes])
    lines = [" ".join([n, *[p.name for p in i.parents if p.name != "*"]])
             for n, i in dag.nodes.items() if n != "*"]
    path.write_text("\n".join(lines) + "\n")
...
def test_role_terms_carry_the_name_and_match_by_the_graph(
    ...
    out = run.ok("give", "ride", "from(my_home)", "my_home", "5")
    assert "give     from(my_home) my_home ride" in out and "→" not in out
    oid = out.strip().splitlines()[-1]
...
def test_regions_and_floors_are_names_in_role_terms(env, tmp_path, monkeypatch):
    ...
    run = Runner()
    out = run.ok("give", "delivery", "my_home_4th", "5")
    assert "give     delivery my_home_4th" in out
    monkeypatch.setenv("LOOP_MAKER", "bruno")
    out = run.ok("want", "delivery", "ljubljana", "6")
    assert "want     delivery ljubljana" in out
    code, out, err = run("matches")
    assert code == 0 and "amara gives delivery my_home_4th to bruno" in out
    ...
    run.ok("give", "delivery", "ljubljana", "5")
    run.ok("want", "delivery", "geo(u2f)", "6")
    matches = run.ok("matches")
    assert "to chen" not in matches                         # nothing at u2f
    assert "chen gives delivery ljubljana to bruno" in matches   # equal
    assert "chen gives delivery ljubljana to amara" not in matches
    # `offers geo(u2e4)`: the floor is inside the cell, and the region
    # contains it — handover coordinates match when one contains the other
    assert len(run.ok("offers", "geo(u2e4)", "--raw").splitlines()) == 3
    assert "ljubljana" not in run.ok("offers", "geo(u2f)", "--raw")   # chen's want only


def test_time_names_need_nothing_from_loopmarket(env, tmp_path, monkeypatch):
    """`odag put autumn 'time(...)'` then time(autumn): the name's value is
    ...
    run = Runner()
    out = run.ok("give", "apple", "autumn", "5")
    # the name stands as spelled (ontodag #15); the graph orders it
    assert "give     apple autumn" in out and "→" not in out
    assert run.session.catalogue.covers(
        "time(2026-09-01T00:00:00Z..2026-12-31T00:00:00Z)", "autumn")
    # relative spellings are input vocabulary, elaborated to fixed UTC
    out = run.ok("give", "apple", "today..+1d", "5")
    assert "note     today..+1d → time(" in out and "T00:00:00Z.." in out


# ---------------------------------------------------------------- the rest of the surface

def test_withdraw_tombstones_only_my_open_offers(loop, monkeypatch):
    oid = loop.ok("give", "apple", "home", "5").strip().splitlines()[-1]
    monkeypatch.setenv("LOOP_MAKER", "bruno")
...
def test_offers_filters_through_satisfies(loop):
    loop.ok("give", "apple", "home", "5")
    loop.ok("give", "piano-lesson", "home", "100")
...
def test_export_import_round_trip_keeps_ids(loop, tmp_path, monkeypatch):
    ids = {loop.ok("give", "apple", "home", "5").strip().splitlines()[-1],
           loop.ok("want", "bicycle", "home", "50").strip().splitlines()[-1]}
    dump = loop.ok("export")
...
def test_set_is_durable_validated_and_fails_closed(env):
    ...
    assert run("set", "terms", "apple(")[0] == 1               # not a term
    assert run("set", "terms", "today..yesterday-ish")[0] == 1  # not a time
    assert run("set", "book", "/plain/path")[0] == 1
    run.ok("set", "valid", "2h")
    run.ok("set", "terms", "home")
    assert cli._read_config() == {"valid": "2h", "terms": "home"}
...
def test_peers_fold_into_every_answer_and_clear_rebases(env, tmp_path, monkeypatch):
    ...
    amara.ok("give", "piano-lesson", "home", "100")
    amara.ok("want", "produce", "local", "weekly", "home", "104")
    monkeypatch.setenv("LOOP_BOOK", f"rs:{tmp_path / 'bruno'}")
    monkeypatch.setenv("LOOP_MAKER", "bruno")
    bruno = Runner()
    bruno.ok("give", "vegetable-box", "home", "50")
    bruno.ok("want", "bicycle-repair", "home", "52")
    monkeypatch.setenv("LOOP_BOOK", f"rs:{tmp_path / 'chen'}")
    monkeypatch.setenv("LOOP_MAKER", "chen")
    chen = Runner()
    chen.ok("give", "bicycle-repair", "home", "80")
    chen.ok("want", "music-lesson", "home", "83")
...
def stage(env, tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("LOOP_TERMS", "home")     # the default place
    run = Runner()
    run.ok("place", "home", "46.05,14.50,5km")
...
TICKET = ["theatre-ticket", "hamlet",
          "time(2026-10-05T19:00:00Z..2026-10-05T22:00:00Z)", "venue"]
RIDE = ["transport", "person", "from(home)", "to(venue)",
        "time(2026-10-05T17:00:00Z..2026-10-05T19:00:00Z)"]
...
def test_drafts_named_numbered_canonical_and_never_in_the_book(stage):
    ...
    assert out.startswith(f"ticket  want geo({venue}) hamlet theatre-ticket "
                          "time(2026-10-05T19:00:00Z..2026-10-05T22:00:00Z)")
    ...
    assert (f"want     geo({venue}) hamlet theatre-ticket "
            "time(2026-10-05T19:00:00Z..2026-10-05T22:00:00Z)") in out
    # re-drafting a name replaces it and keeps its number
    stage.ok("draft", "ticket", "want", "hamlet", "venue")
    assert stage.ok("drafts").startswith(f"ticket  want geo({venue}) hamlet")
    ...
    assert "\n4  want geo(u24) hamlet" in stage.ok("drafts")
...
def test_plus_between_drafts_composes_and_offer_refuses_until_v4(stage, monkeypatch):
    ...
    stage.ok("draft", "kit", "give", "hamlet", "venue", "9")
...
def test_offer_publishes_a_simple_draft_and_removes_it(stage):
    ...
    stage.ok("draft", "seat", "want", "person", "transport", "home", "20")
...
def test_one_line_composed_want_is_the_same_block(stage):
    ...
    assert f"want     geo({cell}) hamlet" in out
    assert f"note     46.1,14.6,2km → geo({cell})" in out
...
def test_offer_line_is_pythons_offer_literal(loop):
    o = cli.offer_from_line("want 2kg apple home time(2026-10-01..2026-10-03) 9",
                            loop.session)
