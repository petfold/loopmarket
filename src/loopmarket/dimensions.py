...
§3): instead of generated bucket/cell category chains, asks are filed under
*exact* parametric terms — the service window as one linear-interval value,
the service cell as one prefix value — and a bid's candidates come from two
native catalogue queries:

- meaning: ``dag.get(wanted_concepts)`` — exact-necessary: an ask whose
...
recall-exact against the baseline ask x bid product — and settlement
...
from .schema import ASK, BID, Offer, TimeWindow
...
class DimensionIndex:
    """Files asks into a derived catalogue copy; answers bid candidates.

    Build one per solve step, like a snapshot: it is cheap relative to the
    O(asks x bids) product it replaces, and regenerating it is what keeps it
    ...
    def file(self, offer: Offer) -> bool:
        """Index an ASK. Returns False (not filed) when its vocabulary is
    ...
    def candidates(self, bid_offer: Offer) -> set[str]:
        """Ask offer-ids that can possibly match `bid_offer`: inside every
        ...
        by_concept = {item.name
                      for item in self._dag.get(list(bid_offer.thing.concepts))}
        if not by_concept:
            return set()
        by_time = {item.name for item in
                   self._dag.get_overlapping(time_term(bid_offer.service))}
...
def candidate_matches_indexed(
    ...
    `DimensionIndex` instead of the full ask x bid product. Yields exactly
    ...
    asks_by_id: dict[str, Offer] = {}
    for offer in offers:
        if offer.kind == ASK and index.file(offer):
            asks_by_id[offer.offer_id] = offer
    for bid_offer in offers:
        if bid_offer.kind != BID:
            continue
        for oid in sorted(index.candidates(bid_offer)):
            match = check_match(asks_by_id[oid], bid_offer, ontology, now=now)
