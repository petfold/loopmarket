...
```
loopmarket  →  ontodag (>=0.3.1)  →  recordstore (>=0.13.1)  →  Swarm (optional)
```
...
2. **Candidate generation is O(asks×bids).** Correct and fine in memory. The scaling path — prefix scans over `idx/{c,t,g}`, then the same exact check — is wired (keys exist) but not used by the baseline solver. The further step, spacetime buckets as generated OntoDAG nodes so one `dag.get({concept, cell, bucket})` intersects all three, is P1 — **updated 2026-07-30**: ontodag's dimension-lattices design (its `docs/DIMENSIONS.md`) supersedes generated buckets with exact parametric terms (virtual `time(a..b)` queries, geohash as a prefix dimension, regions as nodes over cells); see the update note in ARCHITECTURE.md §3 for the adoption rules (no cell multi-parenting; pin the registry version with the root).
...
5. **`bond`, `oracle`, `arbitrator` are carried, not enforced.** They are in the canonical encoding from day one so ids don't churn when P3 lands (bond escrow, oracle adapters, arbitration hooks, bonded ontology assertions — `Ontology.assert_edge` already takes `bond=`).
