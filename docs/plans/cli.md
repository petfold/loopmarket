...
| plumbing | `set`, `export`, `import`, `help`, `--version`; settings `bond` (my deposit: `QTY[UNIT] CATEGORY... VALUE`), `escrow`, `require_point`, `require_cancel`, `ladder`, `require_accepts`, `require_escrows` (admissibility by declaration, the v5 record accepted 2026-09-19; the render shows the deposit and the requirement) |
