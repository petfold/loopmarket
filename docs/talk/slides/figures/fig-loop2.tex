...
\begin{tikzpicture}
  ...
  \node[note, anchor=north, text width=100mm] at ($(A.south)!0.5!(B.south) + (0,-7mm)$) {Walk one apple round the loop: Bruno turns it into $6/4 = 1.5$ oranges, Amara turns those back into $1.5 \times 5/4 = 1.875$ apples.\\[2pt] More comes back than went out $\Rightarrow$ the loop closes, with room to spare.\\[2pt] One exchange that satisfies both offers: \textbf{4 apples for 5 oranges}.};
\end{tikzpicture}
\end{document}
