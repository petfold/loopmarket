...
\begin{tikzpicture}
  ...
  \node[note, text width=70mm] at (0,-36mm) {weights $w = -\log(\text{rate})$; \quad $\prod \text{rate} > 1 \iff \sum w < 0$\\ a profitable loop is a negative cycle; Bellman--Ford finds it in sorted order, deterministically};
\end{tikzpicture}
\end{document}
