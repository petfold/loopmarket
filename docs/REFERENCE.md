...
| `.operator_of(term)` / `.argument(term)` | the operator category a term names (`transport(bicycle)` and bare `transport` → `transport`; None otherwise); the constraints of its argument in ontodag's canonical spelling (`transport(weight(..8000g) small-item)` → `("small-item", "weight(..8kg)")`; bare → `()`; a term the catalogue refuses → `()`, and `known` is False) — the term is ontodag's category kind (#19, 0.26.0) |
