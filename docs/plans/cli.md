...
| plumbing | `set`, `export`, `import`, `help`, `--version`; settings `bond` (my deposit: `QTY[UNIT] CATEGORY... VALUE`, or a bare amount of `default_asset`), `escrow`, `default_asset` (the configured chain's gas token — `xdai xDAI 1` while Swarm settles on Gnosis — a CLI default the record spells out; an asset category the catalogue lacks is refused at publish), `require_point`, `require_cancel`, `ladder`, `require_accepts`, `require_escrows` (admissibility by declaration, the v5 record accepted 2026-09-19; the render shows the deposit and the requirement) |
