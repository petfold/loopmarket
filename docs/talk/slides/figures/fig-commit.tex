...
\begin{tikzpicture}[role/.style={box, align=center, font=\footnotesize, minimum width=30mm, minimum height=13mm}]
  ...
  % only propose; settlement is the one writer of fills; everyone folds.
  ...
  \node[role, fill=accentlight, draw=accent] (T) at (80mm,0) {\textbf{settlement}\\ re-verifies every leg\\ writes \texttt{fill/} + \texttt{loop/}\\ \textbf{one commit, one root}};
  \node[role] (R) at (120mm,0) {\textbf{everyone}\\ folds the settlement feed\\ like any maker's\\ {\color{soft}filled = a record, not a delete}};
  ...
  \node[note, text width=128mm] at (60mm,-42mm) {The commit records obligations; the goods still have to move. Delivery is off-chain and is what P3's bonds, oracles and factbond certificates secure.};
\end{tikzpicture}
\end{document}
