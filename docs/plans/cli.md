...
| plumbing | `set`, `export`, `import`, `help`, `--version`; settings `bond` (my declared bond on every offer) and `require_bond` (the least bond a counterparty must declare — admissibility by declaration, the v5 record, 2026-09-18; the render shows `requires`) |
